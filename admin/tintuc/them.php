...
// Xử lý upload ảnh - upload vào img/ và chỉ lưu tên file
    if ($image) {
        $target_dir = 'img2/';  // Thư mục img/ (đặt ở root hoặc admin/img/ tùy cấu trúc)
        $target_file = $target_dir . basename($image);
        if (move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
            $image = basename($image); // Chỉ lưu tên file: tenanh.jpg
        } else {
            echo "<div class='alert alert-danger'>Lỗi upload ảnh!</div>";
        }
    } else {
        $image = ''; // Nếu không upload ảnh
    }
